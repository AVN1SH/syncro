...
const DeleteConnection = () => {
  ...
  const isModelOpen = isOpen && type === "deleteConnection";
  ...
  useEffect(() => {
    if(isOpen && type === "deleteConnection") {
      setModelOpen(true);
    }
  }, [isOpen]);
      ...
      <Dialog open={modelOpen} onOpenChange={handleOpenChange}>
        <DialogContent className="bg-white text-black p-0 overflow-hidden dark:bg-neutral-800 dark:text-white">
          <DialogHeader className="pt-8 px-6">
            <DialogTitle className="text-2xl text-center font-bold">Delete Connection</DialogTitle>
            <DialogDescription className="text-center text-zinc-500">
              Are you sure You want to Delete <span className=" font-semibold text-red-500">{data?.connectionName}</span>
            </DialogDescription>
          </DialogHeader>
