...
const EditThread = () => {
  ...
  const isModelOpen = isOpen && type === "editThread";
  ...
  useEffect(() => {
    if(data?.thread) {
      form.setValue("name", data?.thread?.name);
      form.setValue("type", data?.thread?.type);
    } else {
      ...
      <Dialog open={modelOpen} onOpenChange={handleClose}>
        <DialogContent className="bg-white text-black p-0 overflow-hidden dark:bg-neutral-800 dark:text-white">
          <DialogHeader className="pt-8 px-6">
            <DialogTitle className="text-2xl text-center font-bold">Edit Thread</DialogTitle>
          </DialogHeader>
          <Form {...form}>
